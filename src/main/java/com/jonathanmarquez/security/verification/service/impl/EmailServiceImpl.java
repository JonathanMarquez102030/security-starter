...
public class EmailServiceImpl implements EmailService {
  ...
  public void sendOtpEmail(String to, String otpCode, Integer expirationMinutes) {
    try {
      ...
      helper.setTo(to);
      helper.setSubject("Verificación de Email - Código OTP");
      helper.setText(buildOtpEmailTemplate(otpCode, expirationMinutes), true);

      mailSender.send(message);
      log.info("Email OTP enviado exitosamente a: {}", to);

    } catch (MessagingException e) {
      log.error("Error al enviar email OTP a {}: {}", to, e.getMessage(), e);
  ...
  /**
   * Construye el template HTML para el email de OTP.
   */
  private String buildOtpEmailTemplate(String otpCode, Integer expirationMinutes) {
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background-color: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                <p style="font-size: 16px; margin-bottom: 20px;">
                    Gracias por registrarte. Para completar tu registro, utiliza el siguiente código de verificación:
                </p>
               \s
               ...
               \s
                <div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 12px; margin: 20px 0; border-radius: 4px;">
                    <p style="margin: 0; font-size: 14px; color: #856404;">
                        ⏱️ Este código expirará en <strong>%d minutos</strong>
                    </p>
                </div>
               \s
